...
namespace CardGame.Editor.DataTableTools
{
    ...
    {
        private const string DataTablePath = "Assets/CardGame/DataTables";
        private const string CSharpCodePath = "Assets/CardGame/Scripts/DataTable";
        private const string CSharpCodeTemplateFileName = "Assets/CardGame/Configs/DataTableCodeTemplate.txt";
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "CardGame");
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.Read{1}();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name)
                            .AppendLine();
                    }
                }
            }
