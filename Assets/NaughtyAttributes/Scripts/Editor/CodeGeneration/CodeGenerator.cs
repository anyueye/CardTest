...
namespace NaughtyAttributes.Editor
{
    public class CodeGenerator : UnityEditor.Editor
    {
        ...
        [MenuItem("SRTK/NaughtyAttributes/Update Attributes Database", false, 0)]
        private static void UpdateDataBase()
        {
            PropertyMetaDatabase.mapper.Update();
            PropertyDrawerDatabase.mapper.Update();
            PropertyGrouperDatabase.mapper.Update();
            PropertyValidatorDatabase.mapper.Update();
            PropertyDrawConditionDatabase.mapper.Update();
            PropertyMetaDatabase.mapper.Update();
            MethodDrawerDatabase.mapper.Update();
            NativePropertyDrawerDatabase.mapper.Update();
        }

        [MenuItem("SRTK/NaughtyAttributes/Log Attributes Database", false, 100)]
        private static void LogDataBase()
        {
            foreach (var kvp in PropertyMetaDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in PropertyDrawerDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in PropertyGrouperDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in PropertyValidatorDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in PropertyDrawConditionDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in PropertyMetaDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in MethodDrawerDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
            foreach (var kvp in NativePropertyDrawerDatabase.mapper.elemByAttributeType) Debug.Log($"Key= {kvp.Key.Name} <==> Value= {kvp.Value.GetType().Name}");
        }
